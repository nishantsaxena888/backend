            ...
            <main className="flex-1 flex flex-col min-w-0 bg-muted/10 pb-20 lg:pb-0">
                <header className="h-16 lg:h-20 border-b border-border bg-background px-4 lg:px-8 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-2 lg:gap-4">
                        ...
                        <h1 className="text-xl lg:text-2xl font-black">Restaurant POS</h1>
                            ...
                            {filteredProducts.map(product => (
                                ...
                                >
                                    <CardContent className="p-0">
                                        <div className="aspect-square relative flex items-center justify-center text-5xl lg:text-6xl bg-orange-500/10 group-hover:bg-orange-500/20 transition-colors">
                                            {product.image}
                                        </div>
                                        <div className="p-3 lg:p-4 space-y-1">
                                            <p className="text-[9px] lg:text-[10px] font-black uppercase text-muted-foreground tracking-widest">{product.category}</p>
            ...
            <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-background border-t shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)] p-4 px-6 z-40 flex items-center justify-between">
                ...
                <Sheet>
                    <SheetTrigger asChild>
                        <Button className="h-14 px-8 rounded-2xl font-black bg-orange-600 hover:bg-orange-700 text-white shadow-lg shadow-orange-500/20">
                            View Order
                        </Button>
                    </SheetTrigger>
                    <SheetContent side="right" className="w-[90%] sm:max-w-[400px] p-0 flex flex-col border-l border-border shadow-2xl">
                        <SheetHeader className="p-6 border-b border-border bg-background text-left flex flex-row items-center gap-3">
                            <div className="w-10 h-10 bg-orange-500/10 rounded-xl flex items-center justify-center text-orange-500 shrink-0 mt-0">
                                <ShoppingBag className="w-5 h-5" />
                            </div>
                            <div className="flex flex-col">
                                <SheetTitle className="text-xl font-black m-0">Current Order</SheetTitle>
                                <span className="text-xs text-muted-foreground font-medium">{cartItems.length} items</span>
                            </div>
                        </SheetHeader>
                        {renderCart(true)}
                    </SheetContent>
