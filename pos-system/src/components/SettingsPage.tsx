        ...
        <div key={currentLanguage.code} className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                ...
                <div className="md:col-span-3 space-y-2">
                    {[
                        { icon: Globe, label: t('General', currentLanguage.code, 'ui'), active: true },
                        { icon: Palette, label: t('Appearance', currentLanguage.code, 'ui') },
                        { icon: Bell, label: t('Notifications', currentLanguage.code, 'ui') },
                        { icon: Shield, label: t('Security', currentLanguage.code, 'ui') },
                        { icon: Database, label: t('Data & Backup', currentLanguage.code, 'ui') },
                    ].map((item, i) => (
                        <Button
                            key={i}
                            variant={item.active ? 'secondary' : 'ghost'}
                            className={`w-full justify-start h-12 rounded-xl font-bold gap-3 ${item.active ? 'text-primary' : ''}`}
                        >
                ...
                <div className="md:col-span-9 space-y-8">
                    ...
                    {/* Advanced Section */}
                    <div className="space-y-4">
                        <h3 className="text-lg font-black tracking-tight">{t('System & Maintenance', currentLanguage.code, 'ui')}</h3>
                        ...
                        <div className="p-6 rounded-[24px] border-2 bg-muted/20 border-dashed space-y-6">
                            <div className="flex items-start gap-4">
                                ...
                                <div className="flex-1">
                                    <h4 className="font-black text-sm">{t('Database Health', currentLanguage.code, 'ui')}</h4>
                                    <p className="text-xs font-bold text-muted-foreground mt-1">
                                        {t('Your local database was last optimized 2 hours ago.', currentLanguage.code, 'ui')}
                                    </p>
                                    <div className="flex gap-4 mt-4">
                                        <Button variant="outline" size="sm" className="h-10 rounded-xl border-2 font-black">{t('Optimize Now', currentLanguage.code, 'ui')}</Button>
                                        <Button variant="outline" size="sm" className="h-10 rounded-xl border-2 font-black text-destructive hover:bg-destructive/10">{t('Clear Cache', currentLanguage.code, 'ui')}</Button>
                                    </div>
                                </div>
                            </div>
                        </div>
