...
const INDUSTRIES = {
    Grocery: ["emerald-grocery", "grey-grocery"],
    Fashion: ["fashion-black", "fashion-gold-luxury"],
    MVP: ["green-mvp"],
    Liquor: ["liqour-black", "liquor-orange"],
    Restaurant: ["restaurant-black"],
    Warehouse: ["warehouse-blue"]
} as const;
...
export function POSContent({ config, products }: POSContentProps) {
    ...
    const currentIndustryEntry = Object.entries(INDUSTRIES).find(([_, themes]) => (themes as readonly string[]).includes(theme));
    const currentIndustry = currentIndustryEntry ? currentIndustryEntry[0] : "Grocery";
    const availableThemesForIndustry = INDUSTRIES[currentIndustry as keyof typeof INDUSTRIES] || INDUSTRIES.Grocery;

    const handleIndustryChange = (newIndustry: string) => {
        setTheme(INDUSTRIES[newIndustry as keyof typeof INDUSTRIES][0] as any);
    };
            ...
            <main className="flex-1 flex flex-col min-w-0 bg-muted/10">
                <header className="h-20 border-b border-border bg-background px-8 flex items-center justify-between shrink-0">
                    ...
                    <div className="flex items-center gap-4">
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-56 rounded-2xl border-border/50">
                                ...
                                {Object.keys(INDUSTRIES).map((indName) => (
                                    <DropdownMenuItem
                                        key={indName}
                                        onClick={() => handleIndustryChange(indName)}
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant="outline" className="rounded-xl font-bold border-border/50 text-muted-foreground hover:text-foreground">
                            ...
                            <DropdownMenuContent align="end" className="w-56 rounded-2xl border-border/50">
                                ...
                                <DropdownMenuSeparator />
                                {availableThemesForIndustry.map((tName) => (
                                    <DropdownMenuItem
                                        key={tName}
